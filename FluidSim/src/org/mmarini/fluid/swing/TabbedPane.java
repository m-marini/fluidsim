...
public class TabbedPane extends JTabbedPane {
	...
	if (comp != null) {
	    ...
	    setToolTipTextAt(getComponentCount() - 1, Messages
		    .getString("TabbedPane.cellTab.tips")); //$NON-NLS-1$
	}
	comp = getRelationComponent();
	if (comp != null) {
	    addTab(Messages.getString("TabbedPane.relationTab.text"), comp); //$NON-NLS-1$
	    setToolTipTextAt(getComponentCount() - 1, Messages
		    .getString("TabbedPane.relationlTab.tips")); //$NON-NLS-1$
	}
	comp = getFluxComponent();
	if (comp != null) {
	    addTab(Messages.getString("TabbedPane.fluxTab.text"), comp); //$NON-NLS-1$
	    setToolTipTextAt(getComponentCount() - 1, Messages
		    .getString("TabbedPane.fluxTab.tips")); //$NON-NLS-1$
	}
    }
